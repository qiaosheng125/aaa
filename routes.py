...
from models import User, File, EncryptedFile, FileStatus, StatusLog, audit_logger, ClientMode, SystemSettings, beijing_now, UserSession, Notification, DeviceRegistry, DecryptionRecord, AuditLog, UserNotificationConfirm
...
# 支持带截止时间的文件名格式（只保留新格式）
PATTERN = re.compile(r"^(.+?)_(.+?)_金额(\d+)元_(\d+)张_(\d{1,2}\.\d{1,2})\.txt$")
...
def check_sequence_continuity(files):
    ...
    for file in files:
        ...
        if client_id and seq_num is not None:
            # 只记录数字序号
            batch_sequence_numbers[client_id].append(seq_num)
    
    # 第二步：对每个客户检查序号是否连续
    for client_id, numbers in batch_sequence_numbers.items():
        # 按序号排序
        numbers.sort()
            ...
            for i in range(len(numbers) - 1):
                if numbers[i + 1] - numbers[i] > 1:
                    ...
                    for missing_num in missing_numbers:
                        warning = f"客户「{client_id}」可能漏上传了第{missing_num}单"
...
def upload_file():
    ...
    for file in files:
        ...
        try:
            ...
            identifier, content, amount, count, deadline = match.groups()
            ...
            # 新增：获取截止时间（假设通过表单字段deadline传递，格式如22.30）
            ...
            if match := re.match(r'^(.+?)_(.+?)_金额(\d+)元_(\d+)张.*?\.txt$', file.filename):
                ...
                # 拼接截止时间
                if deadline:
                    base_filename = f"{base_filename}_{deadline}"
                filename_with_deadline = f"{base_filename}.txt"
            else:
                filename_with_deadline = file.filename
    ...
    try:
        ...
        try:
            ...
            if new_status == FileStatus.COMPLETED.value:
                ...
                db.session.commit()  # 立即提交，确保状态变更及时反映
...
def view_file_content(file_id):
    ...
    try:
        ...
        # 解析文件名，提取截止时间
        match = re.match(r'^(.+?)_(.+?)_金额(\d+)元_(\d+)张_(\d{1,2}\.\d{1,2})\.txt$', file.filename)
        deadline = None
        if match:
...
def get_file_page(file_id, page):
    ...
    # 检查非管理员用户是否允许查看该文件（仅在首次加载时检查）
    if not current_user.is_admin and page == 0 and not file.view_enabled:
        audit_logger.log(
            user=current_user,
            action_type='disabled_view_attempt',
            ip_address=request.remote_addr,
            details={'file_id': file_id, 'page': page}
        )
        return jsonify({'error': '该文件已被禁用查看，请联系管理员重置'}), 403

    try:
        # 如果是客户首次查看（仅第一页时），将view_enabled设置为False（一次性查看）
        if not current_user.is_admin and page == 0 and file.view_enabled:
            file.view_enabled = False
            db.session.commit()
            app.logger.info(f"用户 {current_user.username} 查看文件 {file.filename}，view_enabled已设置为False")
...
def delete_user(user_id):
    ...
    try:
        ...
        # 2. 数据库清理操作
        app.logger.info("开始执行数据库清理操作 - 使用ORM")
        
        # 单独函数尝试执行一个操作，捕获所有错误但不停止流程
        def try_execute(operation, description):
            try:
                operation()
                db.session.commit()
                app.logger.info(f"已完成: {description}")
                return True
            except Exception as e:
                db.session.rollback()
                app.logger.warning(f"操作失败: {description} - {str(e)}")
                return False

        # 清理通知表引用
        try_execute(
            lambda: db.session.execute(
                text("UPDATE notifications SET created_by_id = NULL WHERE created_by_id = :uid"),
                {"uid": user_id}
            ),
            "清理通知表引用"
        )
        ...
        try_execute(
            lambda: db.session.execute(
                text("UPDATE status_logs SET changed_by_id = NULL WHERE changed_by_id = :uid"),
                {"uid": user_id}
            ),
            "清理状态日志引用"
        )
        ...
        # 删除审计日志
        try_execute(
            lambda: AuditLog.query.filter_by(user_id=user_id).delete(),
            "删除审计日志"
        )
        
        # 删除用户会话
        try_execute(
            lambda: UserSession.query.filter_by(user_id=user_id).delete(),
            "删除用户会话"
        )
        
        # 删除设备注册
        try_execute(
            lambda: DeviceRegistry.query.filter_by(user_id=user_id).delete(),
            "删除设备注册"
        )
        ...
        if file_ids:
            try_execute(
                lambda: DecryptionRecord.query.filter(DecryptionRecord.file_id.in_(file_ids)).delete(synchronize_session='fetch'),
                "删除解密记录"
            )
            
            # 3. 删除文件状态日志
            try_execute(
                lambda: StatusLog.query.filter(StatusLog.file_id.in_(file_ids)).delete(synchronize_session='fetch'),
                "删除文件状态日志"
            )
            
            # 4. 删除加密文件
            try_execute(
                lambda: EncryptedFile.query.filter(EncryptedFile.original_file_id.in_(file_ids)).delete(synchronize_session='fetch'),
                "删除加密文件记录"
            )
        
        # 5. 删除文件
        try_execute(
            lambda: File.query.filter_by(client_id=user_id).delete(),
            "删除文件记录"
        )
        
        # 6. 最后删除用户
        user_deleted = try_execute(
            lambda: db.session.delete(user),
            "删除用户记录"
        )
        
        if user_deleted:
            app.logger.info(f"用户 {user.username} (ID: {user_id}) 已成功删除")
            return jsonify({
                'status': 'success', 
            ...
            # 尝试特殊方式删除用户
            app.logger.info("尝试使用特殊方式删除用户")
            
            # 使用原始SQL直接删除用户，跳过外键检查
            # 注意：这是一个危险操作，只有在常规方法失败后才使用
            user_deleted_special = try_execute(
            ...
            if user_deleted_special:
                return jsonify({
                    'status': 'success',
                    'message': f'用户已通过特殊方式删除'
                })
            else:
                return jsonify({
                    'status': 'error',
...
def increment_order_count():
    try:
        # 新增：限制需求单量不能超过最大设备数
        if current_user.order_count >= current_user.max_devices:
            return jsonify({'error': f'需求单量已达上限（{current_user.max_devices}），无法继续增加'}), 400
    ...
    if current_user.is_admin:
        ...
        response = jsonify({
            'files': [{
                ...
                'client': {
                    'client_mode': file.client.client_mode.value if file.client else None
                },
                'amount': float(file.amount),
                'count': file.count,
                'progress': (file.current_page + 1) if file.count else 0,
                ...
                'view_enabled': file.view_enabled,
                'received_at': file.get_received_time().isoformat() if file.get_received_time() else None,
                'cost_time': (int((file.completed_at - file.get_received_time()).total_seconds()) if file.completed_at and file.get_received_time() else None)
...
def check_session():
    ...
    try:
        ...
        # 获取请求头中的X-Request-Timestamp，辅助实现客户端频率限制
        ...
        # 仅在需要时才更新会话活跃时间 (每30分钟一次)，显著降低数据库负担
        now = beijing_now()
        if (now - session.last_seen).total_seconds() > 1800:  # 30分钟 = 1800秒
            session.last_seen = now
            try:
                db.session.commit()
            except Exception as e:
                # 如果更新失败，记录错误但不影响用户体验
                app.logger.error(f"更新会话时间失败: {str(e)}")
                db.session.rollback()
        ...
        # 优化响应，添加更多有用信息并设置缓存控制
        response = jsonify({
            'valid': True,
            'active_sessions': current_user.get_active_sessions_count(),
            'max_devices': current_user.max_devices,
            'session_expires': (session.last_seen + timedelta(days=1)).strftime('%Y-%m-%d %H:%M:%S'),
            'server_time': now.strftime('%Y-%m-%d %H:%M:%S')
        })
        
        # 添加防缓存头信息，但使用更合理的缓存策略
        response.headers['Cache-Control'] = 'private, max-age=5'  # 允许短暂缓存5秒，减轻服务器负担
        response.headers['Pragma'] = 'no-cache'
        response.headers['Expires'] = '0'
...
def before_request():
    ...
    if current_user.is_authenticated:
        ...
        # 使用本地缓存减少会话查询 - 将会话状态存储在Flask的g对象中
        # 只在第一次访问时查询数据库，后续请求复用结果
        if not hasattr(g, 'user_session_checked'):
            # 检查当前设备的会话是否在数据库中存在
            session = UserSession.query.filter_by(
                user_id=current_user.id,
                session_id=current_session_id
            ...
            # 大幅减少会话更新频率，从5分钟提高到15分钟，显著降低数据库负担
            now = beijing_now()
            if (now - session.last_seen).total_seconds() > 900:  # 15分钟 = 900秒
                session.last_seen = now
                try:
                    db.session.commit()
                except Exception as e:
...
def get_notifications():
    ...
    try:
        ...
        if notification:
            confirmed = False
            if current_user.is_authenticated:
                confirmed = UserNotificationConfirm.query.filter_by(user_id=current_user.id, notification_id=notification.id).first() is not None
            result['notification'] = {
                'id': notification.id,
...
def inject_notifications():
    ...
    try:
        ...
        # 如果有通知且用户已确认，则不返回通知
        if notification:
            confirmed = UserNotificationConfirm.query.filter_by(user_id=current_user.id, notification_id=notification.id).first()
            if confirmed:
                return {'notification': None}
        return {'notification': notification}
    except Exception as e:
        app.logger.error(f"获取通知数据时出错: {str(e)}")
        return {'notification': None}
...
@app.route('/api/files/<int:file_id>/reset-view', methods=['POST'])
@login_required
@admin_required
def reset_file_view(file_id):
    """管理员重置文件的view_enabled状态，允许客户再次查看"""
    file = File.query.get_or_404(file_id)
    
    try:
        # 检查是否需要更改状态
        was_changed = not file.view_enabled
        
        # 设置为可查看状态（即使已经是True）
        file.view_enabled = True
        db.session.commit()
        
        # 记录审计日志
        audit_logger.log(
            user=current_user,
            action_type='reset_file_view',
            ip_address=request.remote_addr,
            details={'file_id': file_id, 'filename': file.filename, 'was_changed': was_changed}
        )
        
        app.logger.info(f"管理员 {current_user.username} 重置了文件 {file.filename} 的view_enabled状态，状态变化: {was_changed}")
        ...
        db.session.rollback()
        app.logger.error(f"重置文件查看状态错误: {str(e)}")
        return jsonify({
            'status': 'error',
            'message': f'重置文件查看状态失败: {str(e)}'
        }), 500

@app.route('/api/notifications/confirm', methods=['POST'])
@login_required
def confirm_notification():
    data = request.get_json()
    notification_id = data.get('notification_id')
    if not notification_id:
        return jsonify({'status': 'error', 'message': '缺少通知ID'}), 400
    exists = UserNotificationConfirm.query.filter_by(user_id=current_user.id, notification_id=notification_id).first()
    if not exists:
        confirm = UserNotificationConfirm(user_id=current_user.id, notification_id=notification_id)
