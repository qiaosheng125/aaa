...
class File(db.Model):
    ...
    view_enabled = db.Column(db.Boolean, default=True)  # 客户是否可以点击展示按钮，默认为True
    ...
    def get_received_time(self):
        """获取文件第一次变为RECEIVED的时间，没有则返回None"""
        for log in sorted(self.transactions, key=lambda x: x.changed_at):
            if log.new_status == 'received':
...
# 用户-通知确认表
class UserNotificationConfirm(db.Model):
    __tablename__ = 'user_notification_confirm'
    id = db.Column(db.Integer, primary_key=True)
