...
def parse_filename(filename):
    """
    统一解析文件名格式：标识符_内容_金额XX元_XX张[_截止时间].txt
    返回结构：{
        "identifier": str,
        "content": str,
        "amount": str,
    ...
    """
    # 支持可选的截止时间字段（如_22.30）
    pattern = r"^(.+?)_(.+?)_金额(\d+)元_(\d+)张(?:_(\d{1,2}\.\d{1,2}))?\.txt$"
    match = re.match(pattern, filename)
    if not match:
        raise ValueError("文件名格式错误，请使用：标识符_内容_金额XX元_XX张[_截止时间].txt")
    ...
    return {
        ...
        "deadline": match.group(5) if match.group(5) else None
    }
